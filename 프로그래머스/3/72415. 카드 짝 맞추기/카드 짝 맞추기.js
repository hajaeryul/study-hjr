function solution(board, r, c) {
    ...
    const go = (board, cards, idx, r, c) => {
        ...
        // 다음 카드
        if(d1 < d2) return dist + go(board, cards, idx + 1, x2, y2);
        else return dist + go(board, cards, idx + 1, x1, y1);
    }
    ...
    const dfs = (depth, visited, cur, minDist) => {
        if(depth === cards.length) {
            ...
            minDist[0] = Math.min(minDist[0], go(tmp, cur, 0, r, c));
            return
        }
